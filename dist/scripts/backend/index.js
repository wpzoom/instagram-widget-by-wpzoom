...
jQuery(function ($) {
  ...
  if (typeof zoom_instagram_widget_admin !== 'undefined') {
    ...
    var getLinkedProducts = function getLinkedProducts(mediaId) {
    ...
    var getProductTag = function getProductTag(mediaId, productId) {
      var products = getLinkedProducts(mediaId);
    ...
    var isProductTagged = function isProductTagged(productId) {
    ...
    var findProductIndex = function findProductIndex(products, productId) {
      for (var i = 0; i < products.length; i++) {
        if (products[i].id === productId) {
          return i;
        }
    ...
    var areProductArraysEqual = function areProductArraysEqual(arr1, arr2) {
      if (arr1.length !== arr2.length) return false;
      var sorted1 = arr1.slice().sort(function (a, b) {
        return a.id - b.id;
      });
      var sorted2 = arr2.slice().sort(function (a, b) {
        return a.id - b.id;
      });
      return JSON.stringify(sorted1) === JSON.stringify(sorted2);
    }; // Get current selected product IDs only (for backward compatibility)
    var getCurrentSelectedIds = function getCurrentSelectedIds() {
      return currentSelectedProducts.map(function (p) {
        return p.id;
      });
    }; // Update the tag button on a product item based on tag state
    var updateProductItemButton = function updateProductItemButton($item, productId) {
    ...
    var initProductLinkModal = function initProductLinkModal() {
      ...
      var strings = zoom_instagram_widget_admin.strings || {};
      var modalHTML = '<div id="wpz-insta-product-link-modal" class="wpz-insta-modal" style="display: none;">' + '<div class="wpz-insta-modal-content">' + '<div class="wpz-insta-modal-header">' + '<button type="button" class="wpz-insta-back-to-products" style="display: none;" title="' + (strings.backToProducts || 'Back') + '"><span class="dashicons dashicons-arrow-left-alt2"></span></button>' + '<h2 class="wpz-insta-modal-title">' + (strings.linkProduct || 'Link to Product') + '</h2>' + '<div class="wpz-insta-tagging-header-preview" id="wpz-insta-tagging-header-preview" style="display: none;"></div>' + '<button type="button" class="wpz-insta-modal-close">&times;</button>' + '</div>' + '<div class="wpz-insta-modal-body">' +
      // Product selection view
      '<div class="wpz-insta-product-selection-view">' + '<p class="wpz-insta-product-link-hint">' + (strings.selectProductsHint || 'Select one or more products. Click an item to toggle selection.') + '</p>' + '<div class="wpz-insta-product-search">' + '<input type="text" id="wpz-insta-product-search-input" placeholder="' + (strings.searchProducts || 'Search products...') + '" />' + '</div>' + '<div class="wpz-insta-product-list" id="wpz-insta-product-list"></div>' + '<div class="wpz-insta-product-pagination"></div>' + '</div>' +
    ...
    var resetProductLinkModalView = function resetProductLinkModalView() {
    ...
    var resetTaggingView = function resetTaggingView() {
      isTaggingViewActive = false;
      currentTaggingProductId = null;
      currentSelectedAlbumIndex = null;
      var strings = zoom_instagram_widget_admin.strings || {};
    ...
    var showAlbumSelectorView = function showAlbumSelectorView(productId, productTitle, productImage) {
      var strings = zoom_instagram_widget_admin.strings || {};
      isTaggingViewActive = false;
      ...
      $('#wpz-insta-tagging-product-info').data('product-id', productId).data('product-title', productTitle).data('product-image', productImage);
    ...
    var loadAlbumImages = function loadAlbumImages() {
    ...
    var renderAlbumImages = function renderAlbumImages(images) {
      var $container = $('#wpz-insta-album-images');
      var html = '';
      if (images.length === 0) {
        ...
        images.forEach(function (img, index) {
          ...
          html += '<div class="wpz-insta-album-image-item" data-image-url="' + encodeURIComponent(imageUrl) + '" data-image-index="' + index + '">' + '<img src="' + imageUrl + '" alt="" />' + '<span class="wpz-insta-album-image-number">' + (index + 1) + '</span>' + '</div>';
        });
      }
      $container.html(html);
    }; // Go back to album selector from tagging view
    var showAlbumSelectorFromTagging = function showAlbumSelectorFromTagging() {
      var strings = zoom_instagram_widget_admin.strings || {};
      isTaggingViewActive = false;
    ...
    var showTaggingView = function showTaggingView(productId, productTitle, productImage, imageUrl) {
      var strings = zoom_instagram_widget_admin.strings || {};
      ...
      var headerPreviewHtml = '<div class="wpz-insta-tagging-product-preview">' + '<img src="' + productImage + '" alt="" />' + '<span>' + productTitle + '</span>' + '</div>';
    ...
    var removeCurrentTagPosition = function removeCurrentTagPosition() {
    ...
    var updateRemoveTagPositionButtonVisibility = function updateRemoveTagPositionButtonVisibility() {
      var productId = $('#wpz-insta-tagging-product-info').data('product-id');
    ...
    var placeProductTag = function placeProductTag(productId, x, y) {
      ...
      var $tag = $('<div class="wpz-insta-tag" data-product-id="' + productId + '" data-x="' + x + '" data-y="' + y + '">' + '<span class="wpz-insta-tag-dot"></span>' + '<span class="wpz-insta-tag-label">' + productTitle + '</span>' + '</div>');
      $tag.css({
        left: x + '%',
        top: y + '%'
      });
    ...
    var saveTagPositionFromCoords = function saveTagPositionFromCoords(productId, x, y) {
    ...
    var makeTagDraggable = function makeTagDraggable($tag) {
    ...
    var loadExistingTagPosition = function loadExistingTagPosition(productId) {
      var existingIdx = findProductIndex(currentSelectedProducts, productId);
    ...
    var displayTagOnCanvas = function displayTagOnCanvas(productId, x, y) {
      var $container = $('#wpz-insta-tagging-tags');
      var productTitle = $('#wpz-insta-tagging-product-info').data('product-title') || '';
    ...
    var displayProducts = function displayProducts(products, totalPages, currentPage) {
      ...
      if (products.length === 0) {
        ...
        products.forEach(function (product) {
          ...
          html += '<div class="wpz-insta-product-item' + selectedClass + taggedClass + '" data-product-id="' + product.id + '" data-product-title="' + encodeURIComponent(product.title) + '" data-product-image="' + encodeURIComponent(product.image) + '">' + '<span class="wpz-insta-product-item-checkbox" aria-hidden="true"></span>' + '<img src="' + product.image + '" alt="" />' + '<div class="wpz-insta-product-info">' + '<h4>' + product.title + '</h4>' + '<span class="wpz-insta-product-price">' + product.price + '</span>' + '</div>';
          ...
          if (canTag) {
            ...
            var btnText = hasTag ? strings.editTag || 'Edit Tag' : strings.tagInImage || 'Tag in Image';
            var btnIcon = '<svg xmlns="http://www.w3.org/2000/svg" height="18" viewBox="0 -960 960 960" width="18" fill="currentColor"><path d="M640-760v280l68 68q6 6 9 13.5t3 15.5v23q0 17-11.5 28.5T680-320H520v234q0 17-11.5 28.5T480-46q-17 0-28.5-11.5T440-86v-234H280q-17 0-28.5-11.5T240-360v-23q0-8 3-15.5t9-13.5l68-68v-280q-17 0-28.5-11.5T280-800q0-17 11.5-28.5T320-840h320q17 0 28.5 11.5T680-800q0 17-11.5 28.5T640-760Z"></path></svg>';
            html += "<button type=\"button\" class=\"".concat(btnClass, "\" title=\"").concat(btnText, "\">\n\t\t\t\t\t\t\t").concat(btnIcon, " ").concat(btnText, "\n\t\t\t\t\t\t</button>");
          }
          html += '</div>';
        });
      }
