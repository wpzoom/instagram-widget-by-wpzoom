...
class Wpzoom_Instagram_Widget_API {
    ...
    public static function is_using_basic_display_api($user_id) {
        ...
        $token = get_post_meta($user_id, '_wpz-insta_token', true);

        // Check if token is from Basic Display API (personal) or Business/Creator account
        $is_basic_token = strpos($token, 'IGQ') === false; // Basic Display API tokens don't start with IGQ

        // If using Basic Display API token and not a business/creator account
        return $is_basic_token && (!$account_type || $account_type === 'personal');
