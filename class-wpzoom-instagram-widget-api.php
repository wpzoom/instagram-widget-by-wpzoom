...
class Wpzoom_Instagram_Widget_API {
    ...
    /**
     * Get account insights for an Instagram business account.
     *
     * @since 2.0.0
     *
     * @param int    $account_id The WordPress post ID of the Instagram user.
     * @param array  $metrics    Array of metric names to retrieve.
     * @param string $period     The period type (day, week, month, lifetime).
     * @param int    $days       Number of days/weeks/months to retrieve data for.
     * @return array|WP_Error Processed insights data or WP_Error on failure.
     */
    public function get_account_insights( $account_id, $metrics, $period, $days ) {
        ...
        if ( isset( $data['error'] ) ) {
            error_log( 'Instagram Widget: Insights API Error - ' . print_r( $data['error'], true ) );
            return new WP_Error(
                'api_error',
    ...
    /**
     * Process raw insights data into a structured format.
     *
     * @since 2.0.0
     *
     * @param array $raw_data Raw data from the Instagram API.
    ...
    /**
     * Get business discovery data for an Instagram account.
     *
     * @since 2.0.0
     *
     * @param int $account_id The WordPress post ID of the Instagram user.
     * @return array|WP_Error Business discovery data or WP_Error on failure.
     */
    public function get_business_discovery( $account_id ) {
        ...
        if ( isset( $data['error'] ) ) {
            error_log( 'Instagram Widget: Business Discovery API Error - ' . print_r( $data['error'], true ) );
            return new WP_Error(
                'api_error',
                isset( $data['error']['message'] ) ? $data['error']['message'] : 'Unknown API error'
            );
