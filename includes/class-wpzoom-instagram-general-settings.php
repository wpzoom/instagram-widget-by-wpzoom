...
class WPZOOM_Instagram_General_Settings {
	...
	public function settings_field_clear_data() {
		?>
		<a href="#" id="wpzoom_instagram_clear_data" data-nonce="<?php echo wp_create_nonce( 'wpzoom_instagram_clear_data' ); ?>" class="button button-primary"><?php esc_html_e( 'Delete all Instagram Images from the Media Library', 'instagram-widget-by-wpzoom' ); ?> <span class="wpzoom-loading"></span></a>
		<p class="description">
	...
	public function wpzoom_instagram_clear_data() {
		...
		if ( ! current_user_can( 'manage_options' ) ) {
			wp_die( esc_html__( 'You do not have enough permission to view this page', 'instagram-widget-by-wpzoom' ) );
		}

		// Check if the request is an AJAX request
		if ( ! wp_doing_ajax() ) {
			wp_die( esc_html__( 'This is not an AJAX request', 'instagram-widget-by-wpzoom' ) );
		}

		// Check if the request is a POST request
		if ( 'POST' !== $_SERVER['REQUEST_METHOD'] ) {
			wp_die( esc_html__( 'This is not a POST request', 'instagram-widget-by-wpzoom' ) );
		}

		// Check if the nonce is valid
		if ( ! check_ajax_referer( 'wpzoom_instagram_clear_data', 'nonce', false ) ) {
			wp_die( esc_html__( 'Invalid nonce', 'instagram-widget-by-wpzoom' ) );
		}
		...
		$transient_base = 'zoom_instagram_is_configured';

		foreach ( (array) $get_feeds as $feed ) {
			$feed_id   = isset( $feed->ID ) ? intval( $feed->ID ) : -1;
			$transient = $transient_base . '_' . $feed_id;
		...
		$response = array(
			'message' => esc_html__( 'All images have been removed', 'instagram-widget-by-wpzoom' ),
		);
