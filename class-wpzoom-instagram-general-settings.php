...
class WPZOOM_Instagram_General_Settings {
	...
	public function wpzoom_instagram_clear_data() {
		...
		if ( ! current_user_can( 'manage_options' ) ) {
			wp_die( esc_html__( 'You do not have enough permission to view this page', 'recipe-card-blocks-by-wpzoom' ) );
		}

		// Check if the request is an AJAX request
		if ( ! wp_doing_ajax() ) {
			wp_die( esc_html__( 'This is not an AJAX request', 'recipe-card-blocks-by-wpzoom' ) );
		}

		// Check if the request is a POST request
		if ( 'POST' !== $_SERVER['REQUEST_METHOD'] ) {
			wp_die( esc_html__( 'This is not a POST request', 'recipe-card-blocks-by-wpzoom' ) );
		}

		// Check if the nonce is valid
		if ( ! check_ajax_referer( 'wpzoom_instagram_clear_data', 'nonce', false ) ) {
			wp_die( esc_html__( 'Invalid nonce', 'recipe-card-blocks-by-wpzoom' ) );
		}
